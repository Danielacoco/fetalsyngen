"""Resamples FeTA 2.4 T2w and dseg (rec-mial and rec-irtk) to 0.75mm isotropic, crop-padded to 256³.

Native resolutions vary per subject (~0.43–0.70mm for rec-mial, 0.5mm for rec-irtk).
Resampled directly from originals to avoid chained interpolation artifacts.
"""
...
bids_path = Path("/media/danielaco/HDD2TB/DataDaniela/feta_2.4")
...
for sub in tqdm(subjects):
    ...
    try:
        data = {}
        t2_files = list(sub.glob("anat/*_T2w.nii.gz"))
        label_files = list(sub.glob("anat/*_dseg.nii.gz"))
        ...
        if not data:
            print(f"No files found for {sub.name}, skipping.")
            continue
        ...
        if "label" in data:
            ...
            if losses:
                print(f"  CROP WARNING {sub.name}: {', '.join(losses)}")
