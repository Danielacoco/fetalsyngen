"""Resamples fetal dHCP T2w, T1w and all segmentations to 0.75mm isotropic, crop-padded to 256³.
...
bids_path = Path("/media/danielaco/HDD2TB/DataDaniela/fetalDHCP/resampled05")
out_path = Path("/media/danielaco/HDD2TB/DataDaniela/fetalDHCP/resampled75")

ALL_KEYS = ["T2w", "T1w", "drawem9"]
IMAGE_KEYS = ["T2w", "T1w"]
LABEL_KEYS = ["drawem9"]
...
for sub in tqdm(subjects):
    for ses in sorted(sub.glob("ses-*")):
        ...
        try:
            ...
            t1_files = list(anat.glob("*_desc-restore_T1w.nii.gz"))
